...
// thank you https://en.wikipedia.org/wiki/Modular_exponentiation
uint32_t modular_power(uint32_t b, uint32_t e, uint32_t m)
{
    uint32_t answer = 1;
    while(e>0)
    {
        uint64_t wide_b = b;    // horrible overflow bug
        if(e % 2 == 1) {
            answer = (answer * wide_b) % m;
        }
        b = (b * wide_b) % m;
        e >>= 1;
    }
...
// thank you https://en.wikipedia.org/wiki/Extended_Euclidean_algorithm
template<class T>
struct bezout { T gcd; T x; T y; };

template<class T>
bezout<T> gcd_extended(T a, T b)
{
    T old_r = a, r = b;
    T old_s = 1, s = 0;
    T old_t = 0, t = 1;
    ...
    bezout<T> answer{old_r,old_s,old_t};
    return answer;
}
